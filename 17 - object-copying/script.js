...
// Проверяем: если это не объект (например, число, строка, null)
// то просто возвращаем значение как есть
if (obj === null || typeof obj !== 'object') {
    return obj;
}

// Создаём новый объект или массив
// если исходный объект — массив → создаём []
...
// Для каждого свойства вызываем deepCopy РЕКУРСИВНО
// это нужно, чтобы скопировать вложенные объекты
 copy[key] = deepCopy(obj[key]);
}

// Возвращаем полностью скопированный объект
return copy;
}

/* 
Функция deepCopy выполняет глубокое копирование объекта с помощью рекурсии.
Она проходит по всем свойствам объекта и, если встречает вложенный объект или массив,
вызывает саму себя, чтобы скопировать их. Благодаря этому создаётся полностью независимая копия.
*/